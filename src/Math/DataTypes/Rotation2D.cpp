...
bool Rotation2D::isParallel(const Rotation2D &other) const {
	return MathUtil::almostEqual(Point2D::crossMultiply(this->toPoint(), other.toPoint()), 0.0);
}
...
bool Rotation2D::operator==(const Rotation2D &other) const {
	return distance(other) < MathUtil::doubleEpsilon;
}
...
std::ostream &operator<<(std::ostream &os, const Rotation2D &rotation) {
	return os << rotation.cos_angle << ',' << rotation.sin_angle;
}

std::istream &operator>>(std::istream &in, Rotation2D &rotation) {
	in >> rotation.cos_angle;
	in.ignore();
	in >> rotation.sin_angle;

	return in;
}

std::string Rotation2D::getCSVColumnNames() const {
	return std::string("cos,sin");
}

std::string Rotation2D::toCSV() const {
...
void to_json(json &j, const Rotation2D &rotation) {
	j = json{{"radians", rotation.getRadians()}};
}

void from_json(const json &j, Rotation2D &rotation) {
	rotation = Rotation2D::fromRadians(j.at("radians").get<double>());
}